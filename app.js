...
function layout(){
    ...
    if(state.radar){
        ...
        panel.style.setProperty("--baseTop","47px")
...
if ("ARAD" == data.hudSenderType){
      ...
      if(data.aradarOn){
        ...
        const radarIcon = document.querySelector(".turnIcon")

        if(data.turnDist < 80){
            radarIcon.style.filter = "drop-shadow(0 0 14px red)"
        }else{
            radarIcon.style.filter = "none"
        }
